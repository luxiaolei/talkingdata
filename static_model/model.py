...
flags.DEFINE_string('ckpt_path', 'simpler_crossed_lr5e6', 'model path, default to current directory')

flags.DEFINE_string('train_path', 'train.csv', 'trainning csv path')
...
flags.DEFINE_integer('embed_dim', 35, 'embedding dimension for dnn part')
flags.DEFINE_integer('eval_steps', 4000, 'eval steps for each iter')
flags.DEFINE_integer('last_steps', 0, 'last global steps')
...
dnn_hidden_units = [500, 300, 100] # 'FLAT_embed_30_lr_5e4_layer_3', 'embed_30_lr_1e3_bs_4k_layer3'
...
important_cols = ['phone_brand', 'device_model', 'H9_to_18', '5_4_day',
				'H0_to_9', '5_1_day', 
				'weekend_count', 'weekday_count']
CROSS_LIST_complex = list(combinations(important_cols,2)) + list(combinations(important_cols,3))

CROSS_BASE_COL1 = ['phone_brand', 'device_model']
CROSS_BASE_COL2 = ['H9_to_18', '5_4_day', '5_5_day', '5_6_day','5_7_day', 'weekend_count']
...
CROSS_LIST = [
			...
			['phone_brand', '5_1_day'],
			['phone_brand', '5_4_day'],
			[ 'phone_brand', 'device_model', 'H0_to_9'],
			['5_7_day', '5_1_day'],
			['phone_brand', 'weekend_count', 'weekday_count'],
			['5_1_day', '5_4_day', 'H0_to_9']
			]
"""
CROSS_LIST_complex = [
			['phone_brand', 'device_model'],
...
def model(class_cont=12, dnn_hidden_units= [300, 500, 100, 50]):
	...
	# cross 2-d columns with all CROSS_BASE_COLs
	...
	for col in CROSS_BASE_COL1 + CROSS_BASE_COL2 + CROSS_BASE_COL3:
		for names, crossed in crossed_names:
			if col not in names:
				layer_col = next((layer for layer in cross_base_columns
								if layer.name == col or layer.name == col+'_BUCKETIZED'), None)
				crossed_columns_2d += [tf.contrib.layers.crossed_column([layer_col, crossed], hash_bucket_size=int(1e6))]
	#crossed_columns += crossed_columns_2d
	"""
	for col_list in CROSS_LIST_complex:
...
def train_model(m, df_train, start, stop):
	...
	m.fit(input_fn=lambda: input_fn(df_t), steps=FLAGS.batch_size)
...
def main(_):
	...
	if FLAGS.training:
		...
		for i in range(max_step/FLAGS.batch_size):
			...
			m.evaluate(input_fn=lambda: input_fn(df_train.iloc[train_indx]), steps = 1, name='train')
			m.evaluate(input_fn=lambda: input_fn(df_eval.iloc[eval_indx]), steps = 1, name='valid')
	else:
